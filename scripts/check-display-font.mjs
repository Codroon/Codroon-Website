...
console.log("── the face and its axes ──");
{
  ...
  // The CSS family is "bricolage", generated by next/font/local in
  // layout.tsx. It used to be the fontsource literal "Bricolage
  // Grotesque Variable"; self-hosting the face so it could be preloaded
  // renamed it (client, 2026-08-09). Probing the old name silently
  // measured a system fallback, which has no wdth or opsz axis, so the
  // axis assertions below failed while the axes were in fact fine.
  const r = await p.evaluate(() => {
    const probe = document.createElement("span");
    probe.textContent = "Handgloves 0123456789";
    probe.style.cssText =
      'position:absolute;visibility:hidden;white-space:nowrap;font-family:bricolage;font-size:64px;font-weight:600;';
    document.body.appendChild(probe);
    const w = (s) => { probe.style.fontVariationSettings = s; return +probe.getBoundingClientRect().width.toFixed(2); };
    const out = {
      loaded: document.fonts.check('600 40px bricolage'),
...
console.log("\n── sizes and line heights unchanged by the swap ──");
{
  ...
  for (const k of ["display", "h1", "h2"]) {
    ok(`${k}: display face at wght 600`,
      scale[k].weight === "600" && /bricolage/i.test(scale[k].family), `${scale[k].weight} ${scale[k].family}`);
  ...
  const light = await p.evaluate(() =>
    [...document.querySelectorAll("body *")]
      .filter((e) => {
        ...
        return /bricolage/i.test(cs.fontFamily) && e.textContent.trim() && +cs.fontWeight < 500 && e.offsetParent !== null;
      })
  ...
  for (const path of PAGES) {
    ...
    const r = await p.evaluate(() => {
      ...
      for (const e of document.querySelectorAll("h1, h2, h3, p, dd, span, td, th")) {
        const cs = getComputedStyle(e);
        if (!/bricolage/i.test(cs.fontFamily)) continue;
